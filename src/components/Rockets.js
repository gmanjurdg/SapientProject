...
class Rockets extends React.Component {
  ...
  componentDidMount() {
    ...
    // Get list of past launches and append to array
    axios
  ...
  render() {
    ...
    return (
      <div className="Rockets">
        <Typography variant="h5" style={{marginBottom: '5px', color:"black"}}>SpaceX Rockets</Typography>
        <Typography style={{marginBottom: '5px', color:"black"}}>
          Below are the current, retired, and future vehicles!
        </Typography>
       
        {/* < > buttons for navigation */}
        <IconButton onClick={this.handleChangeBack} ><ArrowBackIosIcon/></IconButton>
        <IconButton onClick={this.handleChangeForward} ><ArrowForwardIosIcon/></IconButton>
        <br/>
        <SwipeableViews index={this.state.index} disabled>
        {
          this.state.rockets.map(rocket => (
            ...
            <Grid item >
            <Typography align="left" variant="h4">{rocket.mission_name}</Typography>
            ...
            <br/>
            <Typography align="left">Flight No: {rocket.flight_number} </Typography>
            <Typography align="left">First flight: {moment(rocket.first_flight).format('D MMM YYYY')}</Typography>
            {/* <Typography align="left">Height: {rocket.payload_mass_kg}m </Typography> */}
            <Typography align="left">Details: {rocket.details}kg</Typography>
            ...
            <Grid item>
              {/* <Typography align="left">Engines: {rocket.number}</Typography> */}
              <Typography align="left">Eng. type: {`${rocket.rocket_type}`.charAt(0).toUpperCase() + `${rocket.rocket_type}`.slice(1)}</Typography>
